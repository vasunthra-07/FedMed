...
import { FileSearch2, History } from "lucide-react";
...
import { usePrescription, useDoctorAction } from "@/lib/hooks/use-prescriptions";
import { useSafetyAnalysis } from "@/lib/hooks/use-safety";
import { useAuditTrail } from "@/lib/hooks/use-audit";
import { getDoctorActions } from "@/lib/workflow/doctor-actions";
import { formatDate } from "@/lib/format";
import type { DoctorActionKey } from "@/lib/api/prescriptions";

export default function DoctorReviewPage({ params }: { params: Promise<{ prescriptionId: string }> }) {
  const { prescriptionId } = use(params);
  const { data: rx, isLoading } = usePrescription(prescriptionId);
  const { data: analysis, isLoading: analysisLoading } = useSafetyAnalysis(prescriptionId);
  const { data: auditEvents, isLoading: auditLoading } = useAuditTrail(prescriptionId);
  const doctorAction = useDoctorAction();

  if (isLoading) return <LoadingState label="Loading prescription…" />;
  if (!rx) return <EmptyState title="Prescription not found" />;

  const actions = getDoctorActions(rx.workflowStatus);

  return (
    <div className="flex flex-col gap-5">
      <PageHeader
        title={`Prescription review · ${rx.displayId}`}
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
                  ...
                  <Card key={med.id}>
                    ...
                    <CardContent className="grid grid-cols-2 gap-x-4 gap-y-2 pb-5 sm:grid-cols-4">
                      ...
                      {med.specialInstructions && (
                        <MedField label="Special instructions" value={med.specialInstructions} className="col-span-2 sm:col-span-4" />
                      )}
                    </CardContent>
                  </Card>
                ))}
                {rx.clinicalNotes && (
                  <Card>
                    ...
                    <CardContent className="pb-5">
                      <p className="whitespace-pre-line text-sm text-muted-foreground">{rx.clinicalNotes}</p>
                    </CardContent>
                  </Card>
                )}
              </div>
        ...
        <div className="flex flex-col gap-3 xl:sticky xl:top-20 xl:self-start">
          ...
          <WorkflowActionPanel
            ...
            actions={actions}
            pending={doctorAction.isPending}
            onAction={(key, note) =>
              doctorAction.mutate({ prescriptionId, action: key as DoctorActionKey, note })
            }
          />
          <Link
            href={`/prescriptions/${prescriptionId}/trace`}
